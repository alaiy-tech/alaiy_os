...
export function Customers({
  ...
  const joinedDateFilter =
    (table.getColumn("joinedWindow")?.getFilterValue() as string | undefined) ??
    "all";
  ...
  const sortValue = React.useMemo(() => {
    ...
    const match = Object.entries(sortOptionState).find(
      ([, state]) =>
        state[0].id === currentSort.id && state[0].desc === currentSort.desc,
    );
      ...
      <CardContent className="space-y-4 pt-0">
        ...
        <div className="flex items-center justify-between px-1">
          <div className="hidden flex-1 text-muted-foreground text-sm lg:flex">
            {table.getFilteredSelectedRowModel().rows.length} of {showing}{" "}
            row(s) selected.
          </div>
          <div className="flex w-full items-center gap-8 lg:w-fit">
            <div className="hidden items-center gap-2 lg:flex">
              <Label
                htmlFor="customers-rows-per-page"
