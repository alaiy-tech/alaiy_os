...
function isPaid(term: SalesOrderPaymentTerm): boolean {
  ...
  if (term.outstanding !== null && term.outstanding !== undefined)
    return term.outstanding <= 0;
            ...
            <TableBody>
              {terms.length === 0 ? (
                ...
                terms.map((term) => {
                  ...
                  const overdue =
                    !paid && !!term.due_date && term.due_date < today;
                      ...
                      <TableCell className="py-3 text-right align-top tabular-nums">
                        <div className="flex flex-col gap-0.5">
                          <span className="font-medium">
                            {money(term.payment_amount)}
                          </span>
                          {!paid && !!term.paid_amount && (
                            <span className="text-muted-foreground text-xs">
                              {money(term.paid_amount)} paid
                            </span>
                          )}
                        </div>
                      </TableCell>
