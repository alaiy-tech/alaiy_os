...
export default async function Page({
  ...
  const [detail, company] = await Promise.all([
    getSalesOrderDetailServer(name),
    getCompanyInfo(),
  ]);
  if (!detail) notFound();
  ...
  const currency =
    detail.order.currency ?? company?.defaultCurrency ?? undefined;
    ...
    <div className="flex flex-col gap-4">
        ...
        action={
          <div className="flex flex-wrap items-center gap-2">
            <OrderActions
              ...
              docstatus={detail.order.docstatus}
            />
      ...
      {cancelled && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            It no longer counts toward delivery or billing. Amend it to carry
            the same lines into a fresh draft.
          </AlertDescription>
        </Alert>
      )}
      ...
      {detail.order.amended_from && (
        <Alert>
          ...
          <AlertDescription>
            <Link
              href={salesOrderHref(detail.order.amended_from)}
