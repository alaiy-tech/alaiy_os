...
import { type ChartConfig, ChartContainer } from "@/components/primitive/chart";
...
function buildGaugeSegments(mix: StockMix, total: number) {
  const inStockSegments = Math.round(
    (mix.in_stock / total) * gaugeSegmentCount,
  );
  const lowStockSegments = Math.round(
    (mix.low_stock / total) * gaugeSegmentCount,
  );
...
export function Inventory({ mix }: { mix: StockMix | null }) {
    ...
    return (
      <InventoryCard availablePercent="—">
        <p className="text-muted-foreground text-sm">
          Could not load inventory. Make sure you&apos;re signed in and try
          again.
        </p>
      </InventoryCard>
    );
  ...
  if (total === 0) {
    return (
      <InventoryCard availablePercent="—">
        <p className="text-muted-foreground text-sm">
          No stock items in the catalog yet.
        </p>
      </InventoryCard>
    );
  }
