...
function ItemFlags({
  ...
  const commerce = COMMERCE_FLAGS.filter(
    (flag) => canWrite || Boolean(item[flag.field]),
  );
...
function ItemDescription({
  ...
  return (
    <EditableField
      ...
      hint={
        hasMarkup
          ? "This description was written with formatting. Saving here replaces it with plain text."
          : undefined
      }
    />
  );
...
export function ItemOverview({
  ...
  const classification = [item.brand, item.item_group].filter(
      ...
      {classification.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 font-medium text-muted-foreground text-xs uppercase tracking-widest">
          {classification.map((part, position) => (
            <span
              key={part}
              className={position === 0 ? "text-foreground" : undefined}
            >
              {position > 0 && (
