...
export default function Products() {
  ...
  const { value: columnPrefs, update: setColumnPrefs } =
    useListPreference<ColumnPrefs>("products:columns", {
  ...
  const [quickFilterId, setQuickFilterId] = useState<string>(
    quickFilters[0]?.id ?? "",
  );
  ...
  const columnFields: ColumnField[] = useMemo(
    () =>
      [...fieldsByName.values()]
        .filter(
          (f) =>
            f.fieldname !== IMAGE_COLUMN_FIELDNAME &&
            f.fieldname !== ID_COLUMN_FIELDNAME,
        )
  ...
  useEffect(() => {
    ...
    const fields = Array.from(
      new Set([
        ...BASE_FIELDS,
        ...contributedFields,
        ...columnPrefs.columnOrder.filter((f) => f !== "status"),
      ]),
    );
    ...
    const quickFilter = quickFilters.find(
      (candidate) => candidate.id === quickFilterId,
    );
    const filters = [
      ...toFrappeFilters(filterRows),
      ...(quickFilter?.filters ?? []),
    ];
    ...
    <Card className="gap-0">
      <CardHeader className="border-b has-data-[slot=card-action]:grid-cols-1 md:has-data-[slot=card-action]:grid-cols-[1fr_auto]">
        ...
        <CardAction className="col-start-1 row-start-auto flex w-full flex-wrap justify-start gap-2 justify-self-stretch md:col-start-2 md:row-span-2 md:row-start-1 md:w-auto md:flex-nowrap md:justify-end md:justify-self-end">
          ...
          <Button
            size="sm"
            onClick={() => toast.info("Add Product is coming soon.")}
          >
            <Plus /> Add Product
          </Button>
        </CardAction>
