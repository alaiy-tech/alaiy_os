  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            {inUse ? `Can't delete ${attribute}` : "Delete attribute"}
          </AlertDialogTitle>
          <AlertDialogDescription>
            {inUse ? (
              <>
                {usageCount === 1 ? "An item is" : `${usageCount} items are`}{" "}
                built on{" "}
                <span className="font-semibold text-foreground">
                  {attribute}
                </span>
                . Remove it from those items first — deleting it would leave
                their variants without a defining attribute.
              </>
            ) : (
              <>
                Permanently delete{" "}
