...
type DialogState =
  | { kind: "none" }
  | { kind: "create" }
  | { kind: "delete"; attribute: string; usageCount: number };
...
function search(attributes: ItemAttributeRow[], term: string) {
  ...
  for (const attribute of attributes) {
    ...
    const valueMatches = attribute.values.some((value) =>
      normalizeValue(value.attribute_value).includes(key),
    );
  ...
  function updateValues(name: string, values: ItemAttributeValue[]) {
    setAttributes((current) =>
      current.map((attribute) =>
        attribute.name === name ? { ...attribute, values } : attribute,
      ),
    );
  }
      ...
      <Card className="gap-0 py-0">
        ...
        <CardContent className="px-0">
          ...
          {!isLoading && !loadFailed && matches.length === 0 && (
            <Empty className="py-10">
              <EmptyTitle>
                {searchTerm ? "No matching attributes" : "No attributes yet"}
              </EmptyTitle>
              <EmptyDescription>
                {searchTerm
          ...
          {!isLoading &&
            ...
            matches.map((attribute) => (
              <AttributeRow
                ...
                attribute={attribute}
                isExpanded={
                  expandedIds.has(attribute.name) ||
                  matchedOnValue.includes(attribute.name)
                }
                onToggle={() => toggleExpand(attribute.name)}
                onValuesChange={(values) =>
                  updateValues(attribute.name, values)
                }
                onDelete={() =>
                  setDialog({
                    kind: "delete",
                    attribute: attribute.name,
                    usageCount: attribute.usage_count,
                  })
                }
              />
            ))}
