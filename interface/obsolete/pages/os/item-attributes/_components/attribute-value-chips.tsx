...
export function AttributeValueChips({
  ...
  const duplicateKeys = findDuplicateKeys(
    values.map((value) => value.attribute_value),
  );

  async function run(
    action: () => Promise<ItemAttributeValue[]>,
    success: string,
  ) {
    setPending(true);
    try {
  ...
  async function handleAdd() {
    ...
    if (
      await run(
        () => addItemAttributeValue(attribute, value),
        `"${value}" was added.`,
      )
    )
      setNewValue("");
  ...
  async function handleRename(row: ItemAttributeValue) {
    ...
    const others = values
      ...
      .map((other) => other.attribute_value);
    ...
    if (
      await run(
        () => renameItemAttributeValue(attribute, row.name, value),
        `Renamed to "${value}".`,
      )
    ) {
      setEditingRow(null);
  ...
  function handleDelete(row: ItemAttributeValue) {
    void run(
      () => deleteItemAttributeValue(attribute, row.name),
      `"${row.attribute_value}" was removed.`,
    );
  }
    ...
    <div className="flex flex-wrap items-center gap-2">
      {values.map((row) => {
        const isDuplicate = duplicateKeys.has(
          normalizeValue(row.attribute_value),
        );
        ...
        return (
          <TooltipWrap
            ...
            label={
              isDuplicate
                ? "Same as another value apart from casing"
          ...
          >
            <Badge
              ...
              className={cn(
                "h-7 gap-0 pr-1 pl-2",
                isDuplicate && "border-destructive/40",
              )}
            >
              <button
                type="button"
