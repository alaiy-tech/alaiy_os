...
export function RenameItemGroupDialog({
  ...
  async function handleRename() {
    ...
    setSubmitting(true);
    try {
      const renamedTo = await renameItemGroup(
        currentName,
        newName.trim(),
        merge,
      );
      toast.success(
        merge ? `Merged into "${renamedTo}".` : `Renamed to "${renamedTo}".`,
      );
        ...
        <div className="flex flex-col gap-4">
          ...
          <Field className="flex-row items-center gap-2 space-y-0">
            <Checkbox
              id="item-group-merge"
              checked={merge}
              onCheckedChange={(checked) => setMerge(checked === true)}
            />
