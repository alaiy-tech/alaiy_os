...
import { type ConnectorRegistryRow, fetchConnectors } from "@/lib/frappe/connectors";
...
function ConnectorCard({ connector }: { connector: ConnectorRegistryRow }) {
  ...
  return (
    <Link
      href={`/os/settings/connectors/${connector.connector_id}`}
      ...
      <Card className="h-full transition-colors hover:border-border hover:bg-muted/40">
        ...
        <CardHeader>
          <CardTitle className="flex min-w-0 items-center gap-2">
            ...
            <span className="min-w-0 break-words">{connector.connector_name}</span>
          </CardTitle>
          ...
          <CardAction>
            <Badge
              variant="outline"
              className={cn("border-0 font-medium capitalize", STATUS_TONE_FOR[status] ?? STATUS_TONE.neutral)}
            >
              {status}
            </Badge>
          </CardAction>
        ...
        <CardContent className="flex flex-wrap items-center gap-x-2 gap-y-1 text-muted-foreground text-sm">
          {connector.connector_type && (
            <Badge variant="outline" className="border-0 bg-muted font-normal text-xs capitalize">
              {connector.connector_type}
            </Badge>
          )}
 ...
 * settings screen at `/os/settings/connectors/<connector_id>`.
 *
...
export function Connectors() {
  const [connectors, setConnectors] = useState<ConnectorRegistryRow[] | null>(null);
  ...
  if (connectors.length === 0) {
    return (
      <Empty className="py-10">
        ...
        <EmptyDescription>
          Installing a connector app registers it here on the next migration. Nothing is installed yet.
        </EmptyDescription>
      </Empty>
    );
  }
