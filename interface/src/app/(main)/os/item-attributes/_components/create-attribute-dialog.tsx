...
import { createItemAttribute, ItemAttributeApiError } from "@/lib/frappe/item-attribute";
...
export function CreateAttributeDialog({
  ...
  async function handleSubmit() {
    ...
    const finalValues = pendingValue && !collidesWith(values, pendingValue) ? [...values, pendingValue] : values;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Attributes define how item variants differ — Colour, Size, Material.</DialogDescription>
        </DialogHeader>
          ...
          <Field className="gap-1.5">
            ...
            <div className="flex items-center gap-2">
              <Input
                ...
                placeholder="e.g. Red"
              />
              <Button variant="outline" size="icon" onClick={addValue} aria-label="Add value">
                <Plus />
              </Button>
            </div>
            ...
            {values.length > 0 && (
              <div className="flex flex-wrap gap-2 pt-1">
                {values.map((value) => (
                  <Badge key={value} variant="secondary" className="h-7 gap-0 pr-1 pl-2">
                    {value}
                    <Button
                      variant="ghost"
                      size="icon-sm"
