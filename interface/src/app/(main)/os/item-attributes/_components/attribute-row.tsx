    ...
    <div className="border-b last:border-b-0">
      <div className="flex items-center gap-2 px-4 py-2.5">
        ...
        >
          ...
          />
          <span className="truncate font-medium text-sm">{attribute.attribute_name}</span>
        ...
        <TooltipWrap label={`Delete ${attribute.attribute_name}`}>
          <Button variant="ghost" size="icon-sm" onClick={onDelete} aria-label={`Delete ${attribute.attribute_name}`}>
            <Trash2 />
          </Button>
        </TooltipWrap>
      </div>
        ...
        <div className="bg-muted/30 px-4 py-3 pl-10">
          {isNumeric ? (
            <p className="text-muted-foreground text-sm">
              Values come from the range {formatAttributeRange(attribute)}. Edit it in ERPNext.
            </p>
          ) : (
            <AttributeValueChips attribute={attribute.name} values={attribute.values} onValuesChange={onValuesChange} />
          )}
        </div>
      )}
    </div>
