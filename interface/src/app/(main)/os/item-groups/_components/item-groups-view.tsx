...
import { asyncDataLoaderFeature, expandAllFeature, hotkeysCoreFeature } from "@headless-tree/core";
import { useTree } from "@headless-tree/react";
import { ChevronsDownUp, ChevronsUpDown, Plus } from "lucide-react";

import { Button } from "@/components/ui/button";
import { getItemGroupChildren, type ItemGroupNode } from "@/lib/frappe/item-group";
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-end gap-2">
        ...
        <Button onClick={() => setDialog({ kind: "create", parentName: rootName, parentLabel: rootName })}>
          <Plus />
          New Item Group
        </Button>
      </div>
      ...
      <ItemGroupTreeView
        tree={tree}
        rootName={rootName}
        onAddChild={(node) => setDialog({ kind: "create", parentName: node.name, parentLabel: node.name })}
        onEdit={(node) => setDialog({ kind: "edit", node })}
        onRename={(node) => setDialog({ kind: "rename", name: node.name })}
        onDelete={(node, parentName) => setDialog({ kind: "delete", name: node.name, parentName })}
      />
      ...
      {dialog.kind === "create" && (
        <ItemGroupFormDialog
          ...
          mode={{ kind: "create", parentName: dialog.parentName, parentLabel: dialog.parentLabel }}
      ...
      {dialog.kind === "edit" && (
        <ItemGroupFormDialog
          ...
          onSaved={() => tree.getItemInstance(dialog.node.name)?.invalidateItemData()}
        />
      )}
