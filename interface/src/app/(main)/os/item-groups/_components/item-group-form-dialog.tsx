...
import { createItemGroup, ItemGroupApiError, type ItemGroupNode, updateItemGroup } from "@/lib/frappe/item-group";

type Mode = { kind: "create"; parentName: string; parentLabel: string } | { kind: "edit"; node: ItemGroupNode };
...
export function ItemGroupFormDialog({
  ...
  async function handleSubmit() {
    ...
    try {
      if (mode.kind === "create") {
        ...
        await createItemGroup({ item_group_name: name.trim(), parent_item_group: mode.parentName, is_group: isGroup });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{isCreate ? "New Item Group" : `Edit ${mode.node.name}`}</DialogTitle>
          {isCreate && <DialogDescription>Under {mode.parentLabel}</DialogDescription>}
        </DialogHeader>
        ...
        <div className="flex flex-col gap-4">
          ...
          <Field className="gap-1.5">
            ...
            />
            {!isCreate && <p className="text-muted-foreground text-xs">Use Rename to change the name.</p>}
          </Field>
        </div>
