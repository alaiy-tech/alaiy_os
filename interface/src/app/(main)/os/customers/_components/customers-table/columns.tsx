...
export const customersColumns: ColumnDef<CustomerRow>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <Badge variant="outline" className={cn("border-0 font-medium", CUSTOMER_STATUS_TONE[row.original.status])}>
        {row.original.status}
      </Badge>
    ),
  },
  {
    accessorKey: "group",
    header: "Group",
    cell: ({ row }) => <span className="text-sm">{orDash(row.original.group)}</span>,
  },
  {
    accessorKey: "territory",
    header: "Territory",
    cell: ({ row }) => <span className="text-sm">{orDash(row.original.territory)}</span>,
  },
  {
    accessorKey: "orders",
    header: "Orders",
    meta: { align: "right" },
    cell: ({ row }) => <span className="text-sm tabular-nums">{row.original.orders.toLocaleString()}</span>,
  },
  {
    accessorKey: "spend",
    header: "Total Spend",
    ...
    cell: ({ row }) => <span className="text-sm tabular-nums">{row.original.spend}</span>,
  },
  {
    ...
    cell: ({ row }) =>
      row.original.lastOrder ? (
        <span className="text-sm">{format(parseISO(row.original.lastOrder), "do MMM yyyy")}</span>
      ) : (
