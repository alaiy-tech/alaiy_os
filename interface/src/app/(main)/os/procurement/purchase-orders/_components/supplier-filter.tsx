...
import { type SupplierOption, searchSuppliers } from "@/lib/frappe/supplier-list";
...
export function SupplierFilter({
  ...
  return (
    <div className="flex items-center">
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent align="start" className="w-[260px] p-0">
          ...
          <Command shouldFilter={false}>
            <CommandInput placeholder="Search suppliers…" value={term} onValueChange={setTerm} />
            <CommandList>
              <CommandEmpty>{isLoading ? "Searching…" : "No suppliers found."}</CommandEmpty>
              <CommandGroup>
                {options.map((option) => (
                  <CommandItem
                    key={option.name}
                  ...
                  >
                    ...
                    <span className="truncate">{option.supplier_name || option.name}</span>
                  </CommandItem>
                ))}
              </CommandGroup>
            </CommandList>
      ...
      {value && (
        <Button variant="ghost" size="icon-sm" onClick={() => onChange(undefined)} aria-label="Clear supplier filter">
          <X />
        </Button>
      )}
    </div>
