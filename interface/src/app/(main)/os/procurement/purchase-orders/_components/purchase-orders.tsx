...
import { fetchPurchaseOrderCount, fetchPurchaseOrders } from "@/lib/frappe/purchase-order-list";
import { getPurchaseOrdersSummary } from "@/lib/frappe/purchase-order-stats";
import { toDateParam } from "@/lib/purchase-orders";
import { useCompany } from "@/stores/company/company-provider";
import type { PurchaseOrderRow, PurchaseOrdersSummary } from "@/types/purchase-orders";

import { buildPurchaseOrderColumns } from "./purchase-order-columns";
import { PurchaseOrderKpiCards } from "./purchase-order-kpi-cards";
import { PurchaseOrderTable } from "./purchase-order-table";
import { SupplierFilter } from "./supplier-filter";

export function PurchaseOrders() {
  const router = useRouter();
  const { meta } = useDoctypeMeta(PURCHASE_ORDER_DOCTYPE);
  ...
  // that predates the Expected Date column and never see it - the new default
  // has to win once.
  const { value: columnPrefs, update: setColumnPrefs } = useListPreference<ColumnPrefs>("purchase-orders:columns:v2", {
    columnOrder: DEFAULT_COLUMN_ORDER,
  });
  const { value: filterRows, update: setFilterRows } = useListPreference<FilterRow[]>("purchase-orders:filters", []);
  ...
  const { filters, orFilters, fromDate, toDate } = useMemo(() => {
    ...
    if (supplier) built.push(["supplier", "=", supplier]);
  ...
  }, [filterRows, statusTab, supplier, dateRange, search]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetchPurchaseOrderCount(filters, orFilters),
    ])
      .then(([data, count]) => {
        if (cancelled) return;
  ...
  }, [filters, orFilters, columnPrefs.columnOrder, sorting, pagination.pageIndex, pagination.pageSize]);

  useEffect(() => {
    let cancelled = false;
    setIsSummaryLoading(true);

    getPurchaseOrdersSummary({ filters, orFilters, fromDate, toDate })
      .then((data) => {
        if (cancelled) return;
        setSummary(data);
  ...
  return (
    <div className="flex flex-col gap-4">
      <PurchaseOrderKpiCards summary={summary} isLoading={isSummaryLoading} hasFailed={summaryFailed} />
      ...
      <Card className="gap-0">
        <CardHeader className="border-b has-data-[slot=card-action]:grid-cols-1 md:has-data-[slot=card-action]:grid-cols-[1fr_auto]">
          ...
          <CardAction className="col-start-1 row-start-auto flex w-full flex-wrap justify-start gap-2 justify-self-stretch md:col-start-2 md:row-span-2 md:row-start-1 md:w-auto md:flex-nowrap md:justify-end md:justify-self-end">
            ...
            <Button size="sm" onClick={() => router.push(`${PURCHASE_ORDER_BASE_PATH}/new`)}>
              <Plus /> New PO
            </Button>
          </CardAction>
        </CardHeader>
        ...
        <CardContent className="flex flex-col gap-0 px-0">
          ...
          <PurchaseOrderTable
            ...
            onRowClick={(row) => router.push(purchaseOrderHref(row.name))}
          />
        </CardContent>
      </Card>
    </div>
