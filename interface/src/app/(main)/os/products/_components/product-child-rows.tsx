...
async function loadVariants(row: ProductRow): Promise<ProductChildRow[]> {
  ...
  return variants.map((variant) => ({
    ...
    tags: variant.attributes.map((a) => ({ label: a.attribute, value: a.attribute_value })),
  }));
}

function ChildTags({ tags, spec }: { tags?: ProductChildRow["tags"]; spec?: string | null }) {
  if (spec) return <span className="truncate text-muted-foreground text-sm">{spec}</span>;
  if (!tags?.length) return <span className="text-muted-foreground">—</span>;
  return (
    <div className="flex flex-wrap gap-1">
      {tags.map((tag) => (
        <span key={tag.label} className="rounded-md bg-muted px-1.5 py-0.5 text-xs">
...
function ChildAmount({ row, currency }: { row: ProductChildRow; currency?: string }) {
  if (row.amount === null || row.amount === undefined) return <span className="text-muted-foreground">—</span>;
...
export function ProductChildRows({
  ...
  const emptyMessage = productExtension?.childrenEmptyMessage ?? "This template has no variants yet.";
                ...
                >
                  <div className="flex min-w-0 items-center gap-3">
                    <div className="flex size-8 flex-none items-center justify-center overflow-hidden rounded-md border bg-muted">
                      {child.image ? (
                        <img src={child.image} alt={child.itemName} className="size-full object-cover" />
                      ) : (
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="truncate text-muted-foreground text-xs">{child.itemCode}</div>
                    </div>
                  </div>
