...
export function SelectionActionsMenu({ selectedIds }: { readonly selectedIds: string[] }) {
  const [deleteOpen, setDeleteOpen] = useState(false);
  const count = selectedIds.length;

  function copyIds() {
    navigator.clipboard.writeText(selectedIds.join(", "));
    toast.success(count === 1 ? "Copied ID to clipboard." : `Copied ${count} IDs to clipboard.`);
  }
  ...
  return (
    <>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete {count === 1 ? "this product" : `these ${count} products`}?</AlertDialogTitle>
            <AlertDialogDescription>
              {count === 1
