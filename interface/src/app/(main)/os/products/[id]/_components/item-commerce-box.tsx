...
import { Boxes, CalendarClock, LifeBuoy, ShieldCheck, Truck } from "lucide-react";

import { Badge } from "@/components/ui/badge";
import { DEFAULT_STOCK_STATE_BADGE_CLASS, getStockStateBadgeClass } from "@/constants/products";
...
function Fact({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <>
      <span className="self-center whitespace-nowrap text-muted-foreground text-xs">{label}</span>
      <span className="min-w-0 break-words font-medium text-sm">
        {value?.trim() ? value : <span className="font-normal text-muted-foreground">{EM_DASH}</span>}
      </span>
    </>
  );
}
...
function orderingSignals(item: ItemDetailHeader) {
  return [
    item.lead_time_days
      ? { key: "lead", icon: <Truck className="size-4" />, label: "Lead time", value: `${item.lead_time_days} days` }
      : null,
    item.min_order_qty
      ? {
          key: "moq",
          ...
          value: `${formatQty(item.min_order_qty)} ${labelOr(item.stock_uom, "")}`.trim(),
        }
      : null,
    item.safety_stock
      ? {
          ...
          label: "Safety stock",
          value: `${formatQty(item.safety_stock)} ${labelOr(item.stock_uom, "")}`.trim(),
        }
      : null,
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-4 rounded-xl bg-muted/60 p-4">
        ...
        <div className="grid grid-cols-2 gap-3 border-t pt-3">
          <div className="flex min-w-0 flex-col gap-1">
            ...
            <span className="truncate font-medium text-sm tabular-nums">{money(item.valuation_rate) ?? EM_DASH}</span>
          </div>
          <div className="flex min-w-0 flex-col gap-1">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Badge
          ...
          className={cn(
            ...
            canReadStock ? getStockStateBadgeClass(stockState) : DEFAULT_STOCK_STATE_BADGE_CLASS,
          )}
        >
      ...
      {signals.length > 0 && (
        <div className="flex flex-col gap-3 border-t pt-4">
          {signals.map((signal) => (
            <Signal key={signal.key} icon={signal.icon} label={signal.label} value={signal.value} />
          ))}
        </div>
      )}
    </div>
