  ...
  const specs: SpecRow[] = [
    {
      ...
      reason: "The document's own name, set by the Item naming series when it was created.",
    },
    { label: "Item Group", field: "item_group", kind: "link", linkDoctype: "Item Group", value: item.item_group },
    { label: "Brand", field: "brand", kind: "link", linkDoctype: "Brand", value: item.brand },
    { label: "Stock UOM", field: "stock_uom", kind: "link", linkDoctype: "UOM", value: item.stock_uom },
    { label: "Purchase UOM", field: "purchase_uom", kind: "link", linkDoctype: "UOM", value: item.purchase_uom },
    { label: "Sales UOM", field: "sales_uom", kind: "link", linkDoctype: "UOM", value: item.sales_uom },
    {
      label: "Standard Rate",
      field: "standard_rate",
      kind: "number",
    ...
    },
    {
      ...
      reason: "Derived by ERPNext from this item's stock movements — not a figure to type.",
    },
    {
    ...
    },
    {
      ...
      display: item.min_order_qty === null ? null : formatQty(item.min_order_qty),
    },
    {
      label: "Safety Stock",
      field: "safety_stock",
    ...
    },
    {
      ...
      display: item.weight_per_unit ? `${item.weight_per_unit} ${item.weight_uom ?? ""}`.trim() : null,
    },
    {
        ...
        <Table>
          <TableBody>
            {specs.map((row, index) => (
              <TableRow
                key={row.label}
                className={index % 2 ? "bg-muted/40 hover:bg-muted/40" : "hover:bg-transparent"}
              >
                <TableCell className="w-1/3 px-4 py-2.5 font-medium">{row.label}</TableCell>
