...
import { DEFAULT_STATUS_BADGE_CLASS, STATUS_BADGE_CLASS } from "@/constants/sales-orders";
...
export function OrderSummary({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="font-normal text-muted-foreground text-sm">Order</CardTitle>
      ...
      <CardContent className="flex flex-col gap-6">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
          <Field label="Status">
            <Badge
              variant="outline"
              className={cn("border-0 font-medium", STATUS_BADGE_CLASS[order.status] ?? DEFAULT_STATUS_BADGE_CLASS)}
            >
              {order.status}
            </Badge>
          </Field>
          ...
          {order.sales_channel && <Field label="Channel">{order.sales_channel}</Field>}
        </div>

        <Separator />

        <OrderProgress order={order} deliveryNoteCount={deliveryNoteCount} invoiceCount={invoiceCount} />
      </CardContent>
    </Card>
  );
}
