...
export function OrderActions({ name, docstatus }: { name: string; docstatus: number }) {
  ...
  async function createAndOpen(key: string, action: () => Promise<{ name: string; doctype: string }>, label: string) {
    setPending(key);
    try {
      ...
      router.push(`/os/open/${created.doctype.toLowerCase().replace(/\s+/g, "-")}/${encodeURIComponent(created.name)}`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Something went wrong.");
      setPending(null);
    }
    ...
    <div className="flex flex-wrap items-center gap-2">
      {docstatus === 0 && (
        <Button
          size="sm"
          disabled={busy}
          onClick={() => run("submit", () => submitSalesOrder(name), `${name} was submitted.`)}
        >
      ...
      {docstatus === 1 && (
        <>
          <Button
            size="sm"
            disabled={busy}
            onClick={() => createAndOpen("delivery-note", () => makeDeliveryNote(name), "Delivery Note")}
          >
          ...
          <Button
            ...
            variant="outline"
            disabled={busy}
            onClick={() => createAndOpen("sales-invoice", () => makeSalesInvoice(name), "Sales Invoice")}
          >
      ...
      <Button asChild variant="outline" size="sm">
        <a href={`${salesOrderHref(name)}/print`} target="_blank" rel="noopener noreferrer">
          <Printer /> Print
        </a>
      </Button>
      ...
      <AlertDialog open={confirmingCancel} onOpenChange={setConfirmingCancel}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This reverses the order in ERPNext — reserved stock is released and the order stops counting toward your
              sales figures. It can be amended into a new draft afterwards, but not un-cancelled. Any submitted delivery
              note or invoice raised against it has to be cancelled first.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                ...
                void run("cancel", () => cancelSalesOrder(name), `${name} was cancelled.`);
              }}
            >
              Cancel order
            </AlertDialogAction>
