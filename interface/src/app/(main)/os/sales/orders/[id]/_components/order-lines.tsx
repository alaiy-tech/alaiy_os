...
function lineDescription(item: SalesOrderLine): string | null {
  const text = (item.description ?? "")
    ...
    .trim();
  if (!text) return null;
  if (text === item.item_name?.trim() || text === item.item_code.trim()) return null;
  return text;
}

export function OrderLines({ items, currency }: { items: SalesOrderLine[]; currency?: string }) {
          ...
          <Table className="w-full **:data-[slot='table-cell']:px-4 **:data-[slot='table-head']:px-4">
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="py-3 text-right font-medium">Delivered</TableHead>
            ...
            <TableBody>
              {items.length === 0 ? (
                ...
                items.map((item) => {
                  ...
                  const outstanding = Math.max(item.qty - item.delivered_qty, 0);
                  ...
                  return (
                    <TableRow key={item.name} className="border-border/60">
                      ...
                      <TableCell className="py-3 text-right align-top">
                        <CoverageCell covered={item.delivered_qty} ordered={item.qty} />
                      </TableCell>
                      <TableCell className="py-3 text-right align-top">
