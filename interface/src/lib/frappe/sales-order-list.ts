export type SalesOrderListRow = Record<string, unknown> & { name: string };
...
export async function fetchSalesOrders(params: SalesOrderListParams): Promise<SalesOrderListRow[]> {
  ...
  const res = await fetch(`/api/resource/Sales Order?${query.toString()}`);
  if (!res.ok) throw new Error(`Failed to fetch sales orders: ${res.status}`);
  const data = (await res.json()) as { data: SalesOrderListRow[] };
  return data.data;
}
