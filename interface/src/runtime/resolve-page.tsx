...
import { baseComponentRegistry } from "./component-registry";
...
import { validateAgainstRegistry } from "./validate-against-registry";
...
/**
 * The one pipeline both the static `/os/headless` route and the dynamic
 * `/os/[...page]` catch-all call: resolve a page id through the current
 * `UIPageStore`, resolve whatever data sources its definition references
 * (via the Data Source Registry - `resolvePageData`), and render through
 * `UIRenderer` (or that page's `pageFeatures` render override, if any). A
 * missing config and an invalid one are reported distinctly - the caller
 * decides how to present each, but neither one ever throws past this
...
export async function resolvePage(id: string, searchParams: SearchParams): Promise<ResolvePageResult> {
  ...
  const registry = baseComponentRegistry;
