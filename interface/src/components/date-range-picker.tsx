...
export function DateRangePicker({
  ...
  return (
    <div className="flex items-center">
      <Popover>
        <PopoverTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            className={cn("justify-start font-normal", !hasValue && "text-muted-foreground", className)}
          >
      ...
      {hasValue && (
        <Button variant="ghost" size="icon-sm" onClick={() => onChange(undefined)} aria-label="Clear date range">
          <X />
        </Button>
      )}
    </div>
