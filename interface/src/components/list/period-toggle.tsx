...
import { PERIODS, type Period, readPeriod } from "./period";
...
export function PeriodToggle({
  ...
  const period = readPeriod(Object.fromEntries(searchParams.entries()), paramName, defaultPeriod);
      ...
      {PERIODS.map((p) => (
        <button
          ...
          className={cn(
            ...
            p === period ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground",
          )}
        >
          {p}
        </button>
