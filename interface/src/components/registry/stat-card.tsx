...
import { Card, CardAction, CardContent, CardDescription, CardHeader } from "@/components/primitive/card";
...
export function StatCard({
  ...
  return (
    <Card className={cn("relative h-full overflow-hidden rounded-none border-0 border-border ring-0", className)}>
      {borderTone && (
        <span aria-hidden className={cn("absolute inset-y-0 left-0 w-1", KPI_BORDER_TONE_CLASSES[borderTone])} />
      )}
      <CardHeader>
      ...
      <CardContent className="space-y-2">
        <div className="flex items-center gap-3">
          <span className="text-3xl text-foreground tabular-nums leading-none tracking-tight">{value}</span>
          {delta}
        </div>
