...
describe("component registry", () => {
  ...
  it("two registries declaring the same type collapse to the last one, not a field-level merge", () => {
    ...
    expect(merged["os-kpi"]).toBe(entryB);
    expect(merged["os-kpi"]).not.toBe(entryA);
  });

  it("every base entry declares a name, category, and explicit ai.exposed", () => {
    for (const type of Object.keys(baseComponentRegistry)) {
      const entry = resolveComponent(baseComponentRegistry, type);
      expect(typeof entry?.name).toBe("string");
      expect(entry?.name?.length).toBeGreaterThan(0);
      expect(entry?.category).toBeDefined();
      expect(entry?.ai?.exposed).toBe(true);
    }
  });
  ...
  it("listAiExposedComponents excludes an entry marked ai.exposed: false and one that omits ai entirely", () => {
    ...
    expect(exposed[0]?.type).toBe("os-kpi");
  });
