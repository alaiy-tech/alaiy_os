...
import { HEADLESS_CUSTOMERS_PAGE, HEADLESS_DASHBOARD_PAGE, HEADLESS_SUPPLIERS_PAGE } from "@/seeds/seed";
 ...
 * production seed pages (`/os`, `/os/customers`, `/os/suppliers`) - the
 * same base registry and the same real, registered Data Source ids, no
 * fixtures. If a future edit to `seed.ts`, the base registry, or the
 * layout span/columns tables ever makes any of them fail this gate, this
 * test catches it without needing a live server or a Frappe session.
 */
describe("validateAgainstRegistry against the real seed pages", () => {
  ...
  it("HEADLESS_SUPPLIERS_PAGE passes with zero errors", () => {
    expect(
      validateAgainstRegistry(HEADLESS_SUPPLIERS_PAGE, {
        componentRegistry: baseComponentRegistry,
        isDataSourceRegistered,
      }),
    ).toEqual([]);
