...
```text
@alaiy-os/primitive/*        → src/components/primitive/*
...
own (`@/components/primitive/button` ↔ `@alaiy-os/primitive/button`), and the public surface
...
database-driven the same round Settings moved to its own route group (see
`docs/UI_RUNTIME.md`'s "Sidebar Store" section). `sidebar-seed.ts`'s merge
function now runs at database-sync time (on every app start, so a redeploy's
updated `contributed-nav.ts` still takes effect with no manual step) instead
of at module-import time, but the merge semantics themselves — matching by
group `label`, an unrecognised label opening a new group, an id collision
within a group replacing rather than duplicating — are unchanged.
...
* [src/app/(main)/settings/connectors/page.tsx](src/app/\(main\)/settings/connectors/page.tsx)
  — Settings → Connectors (moved from `/os/settings/connectors` to
  `/settings/connectors` 2026-08-25, see §16.1), a card per row in
  `OS Connector Registry`, each linking to `/settings/connectors/<connector_id>`.
  The base ships the index and none of the screens behind it; a connector that
  registers without one shows up here as a card whose link goes nowhere, which
  is the point of having an index.
...
* **A connector owns its settings screen, not the base — but the base owns the
  ...
  `/settings/connectors/<id>` rendering a form off field metadata. Half of
  ...
  belongs at `src/app/(main)/settings/connectors/<connector_id>/page.tsx`, so
  ...
  **Contract change, 2026-08-25**: this address moved from
  `/os/settings/connectors/<connector_id>` to `/settings/connectors/<connector_id>`
  when Settings became a top-level route group with its own sidebar, separate
  from `/os/*` (which itself became database-driven the same round - see
  `docs/UI_RUNTIME.md`'s "Sidebar Store" section). This is a breaking change
  for any already-composed connector app targeting the old address -
  `alaiy_os_connector_nayaglobal` (§24, the one documented example) needs a
  matching update in its own repo, not made here.
