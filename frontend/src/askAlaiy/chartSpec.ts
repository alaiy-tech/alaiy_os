 ...
 *
 * The shape is shared-labels -- one `labels` array plus parallel
 * `series[].points` -- rather than [{x, y}] objects. Fewer tokens for the
 * model to write, one zip away from Recharts' row shape, and it makes the
 * table view a plain labels x series grid.
 */
...
export interface ChartSeries {
  name: string;
  /** Same length as `labels`. null is a gap, never a zero. */
  points: (number | null)[];
}
...
export interface ChartSpec {
  ...
  y?: string;
  unit?: ChartUnit;
  /** Set by the parser, not the model: how many labels were dropped. */
  truncated?: number;
}

export const MAX_SERIES = 4;
export const MAX_LABELS = 60;
/** A pie is unreadable past this, and gets downgraded to a bar instead. */
...
/**
 * A spec from the raw fence body, or null if there isn't a usable one.
 * Clamps wherever clamping leaves something honest and rejects only when
...
/**
 * The chart to actually draw. This overrides the model rather than obeying
 * it -- a stated type the data cannot carry is a mistake with a correct
 * answer.
 */
