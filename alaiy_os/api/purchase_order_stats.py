...
def _order_totals(start, end=None, docstatus=None):
	"""Count + sum(grand_total) of Purchase Orders booked in [start, end) by
	transaction_date - end=None means unbounded (up to now)."""
	conditions = ["po.transaction_date >= %(start)s"]
	params = {"start": start}
	if end:
		conditions.append("po.transaction_date < %(end)s")
		params["end"] = end
	if docstatus is not None:
		conditions.append("po.docstatus = %(docstatus)s")
		...
		conditions.append("po.docstatus in (1, 2)")

	row = frappe.db.sql(
		f"""
		select count(*) as cnt, coalesce(sum(po.grand_total), 0) as total
...
@frappe.whitelist()
def get_purchase_orders_overview(period="1M"):
	"""The four Purchase Orders KPI cards (total orders, total order value,
	average order value, cancelled orders) in one call, each as
	{current, previous} for the period-over-period % badge.

	Mirrors sales_order_stats.get_sales_orders_overview so the two pages read
	the same way: total order value and AOV come from submitted (docstatus=1)
	orders only - a cancelled order's grand_total was never really committed
	spend, so including it would inflate both figures.
	"""
