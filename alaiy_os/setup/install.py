...
Reconciles the site's workspace to match this codebase. Company creation,
branding and locale/defaults setup are no longer done here — they're
collected interactively by the /os/onboarding wizard (see
alaiy_os/www/os/onboarding.html and alaiy_os/api/onboarding.py) and applied
once, on completion.
...
# True shows the "Getting Started" panel in the OS workspace sidebar.
...
def restrict_foreign_workspaces():
    ...
    are left untouched. (Welcome Workspace itself gets hard-deleted by the
    onboarding wizard's completion handler — see alaiy_os/api/onboarding.py —
    this just covers everything else, and Welcome Workspace too on any site
    where onboarding hasn't completed yet.)
    """
...
def _get_default_company():
    # Empty until the /os/onboarding wizard completes (see alaiy_os/api/onboarding.py)
    # — the bare WORKSPACE_NAME/SETTINGS_WORKSPACE_NAME fallbacks below cover
    # that pre-onboarding window; title updates to the real company name the
    # next time provisioning runs after onboarding completes.
...
def create_or_update_workspace_sidebar():
    ...
    enable_onboarding = ENABLE_MODULE_ONBOARDING
...
def create_or_update_onboarding():
    ...
    """
    enable = ENABLE_MODULE_ONBOARDING
