...
def build_runnable(agent_id):
	...
	# Check what the tools declare against the user this run will actually read
	# as. api/agent_settings.list_agents shows the same answer to an operator,
	# but that is an answer about a moment: a role revoked since, or a Run As
	# User changed since, would otherwise surface as a Success run full of zeros
	# rather than a failure, because `frappe.get_list` returns an empty set for a
	# user without permission instead of raising.
	#
