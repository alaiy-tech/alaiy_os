...
PRESENTATION_PROMPT = (
	...
	"create_presentation tool. It has title, bullets, table AND chart slides — "
	"a real bar/line/pie chart, not a table of numbers.\n"
	...
	"said so. Do not spend tool calls perfecting the underlying data first: build "
	"the deck from what you can retrieve efficiently, note in prose if anything "
	"was too large to include in full, and offer create_download for the rest. "
	"Running out of turns gathering data instead of building the deck is a "
	"worse outcome than a deck built from a slightly smaller pull.\n"
	"- Make it a real presentation, not a wall of bullets: a title slide, then a "
	"mix of bullets, chart and table slides — reach for a chart whenever the "
	"point is a trend, a comparison, or a share of a total rather than the exact "
	"numbers, since that is what makes a deck look designed instead of like "
	"chat prose split across pages. Write bullets as short, presentable "
	"phrases, not full sentences copied from your prose answer, and keep table "
	"slides for genuinely small tables (a slide is read at a glance, not "
	"scrolled).\n"
